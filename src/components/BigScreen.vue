<template>
  <div id="bigScreen">
    ...
    <div class="main">
      <div class="left">
        <div class="cityEvent" v-for="(item, index) in props.dataInfo">
          <h3>{{ item.name }}</h3>
          <h1>
            <img src="@/assets/bg/bar.svg" class="icon" alt="">
            <span>{{ parseInt(item.number) }}（{{ item.unit }}）</span>
          </h1>
      ...
      <div class="right">
        <div class="cityEvent list">
          ...
          <ul>
            <li v-for="(item, i) in props.eventList" :class="{ active: currentActive == i }" @click="titleClick(i)">
              <h1>
                <div>
